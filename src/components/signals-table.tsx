...
import { Trophy, User, ChevronRight, Filter, ChevronDown } from "lucide-react"
import { cn } from "@/lib/utils"

const signals = [
    { match: "Djokovic vs Musetti", tour: "ATP BNL Roma Masters", date: "Abril 26, 2026 - Clay", odds: "1.42", stake: "$250", prob: "81%", active: true },
    { match: "Gauff vs Pegula", tour: "WTA Surface ELO", date: "Abril 26, 2026 - Clay", odds: "1.42", stake: "$250", prob: "75%" },
    { match: "Heat vs Celtics", tour: "NBA", date: "Abril 26, 2026", odds: "1.45", stake: "$250", prob: "68%" },
    { match: "Villarreal vs Barcelona", tour: "LaLiga", date: "Abril 26, 2026", odds: "1.29", stake: "$250", prob: "92%" },
    { match: "Bucks vs Knicks", tour: "NBA", date: "Abril 26, 2026", odds: "1.90", stake: "$250", prob: "54%" },
]
        ...
        <div className="podio-card flex-1 min-h-[500px] flex flex-col">
            <div className="p-4 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
                <div className="flex items-center gap-6">
                    <h2 className="text-xs font-black uppercase tracking-[0.2em] text-white">Quantum Signals Alpha</h2>
                    <div className="flex items-center gap-1 bg-black/40 p-1 rounded-lg border border-white/5">
                        <button className="px-3 py-1 text-[9px] font-black uppercase bg-emerald-500 text-black rounded shadow-[0_0_10px_rgba(16,185,129,0.3)] transition-all">Node-T</button>
                        <button className="px-3 py-1 text-[9px] font-black uppercase text-gray-600 hover:text-gray-400">Node-B</button>
                        <button className="px-3 py-1 text-[9px] font-black uppercase text-gray-600 hover:text-gray-400">Node-F</button>
                    </div>
                </div>
                ...
                <table className="w-full text-left text-[11px]">
                    <thead>
                        <tr className="uppercase tracking-widest text-gray-700 border-b border-white/5 bg-white/[0.01]">
                            <th className="px-6 py-4 font-black">Identidad / Nodo</th>
                            <th className="px-6 py-4 font-black">Sector</th>
                            <th className="px-6 py-4 font-black">Odd</th>
                            <th className="px-6 py-4 font-black">Allocation</th>
                            <th className="px-6 py-4 font-black text-center">Confidence</th>
                            <th className="px-6 py-4 font-black text-right">Análisis</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                        {signals.map((signal) => (
                            <SignalRow key={signal.match} signal={signal} />
                        ))}
                    </tbody>
                </table>
            </div>
            ...
            <div className="p-4 border-t border-white/5 bg-white/[0.01] flex items-center justify-between mt-auto">
                <span className="text-[9px] font-bold text-gray-700 uppercase tracking-widest">Active Instances: {signals.length}</span>
    ...
    return (
        <tr className={cn(
            ...
            <td className="px-6 py-4 text-right">
                <button className="px-4 py-1.5 rounded-lg border border-white/10 text-[9px] font-black uppercase text-gray-600 group-hover:text-white group-hover:bg-emerald-500/20 group-hover:border-emerald-500/50 transition-all font-sans">
                    Audit Node
                </button>
            </td>
        </tr>
